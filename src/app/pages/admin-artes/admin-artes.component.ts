...
@Component({
  selector: 'app-admin-artes',
  templateUrl: './admin-artes.component.html',
  providers: [MessageService],  
  styleUrls: ['./admin-artes.component.scss']
})
export class AdminArtesComponent implements OnInit, OnDestroy {
  ...
  private getEnterprisesPageableData(params: any = { page: 0, size: 5, tipoSolicitud: 'artes' }) {
  ...
  handleActionTriggered(event: { action: string, data: any }) {
    switch(event.action) {
      case 'view':
        ...
        this.router.navigate(['dashboard/admin-artes-info']);
        break;
